...
describe('TaskScheduler', () => {
  ...
  it('应该遵循优先级顺序执行任务', async () => {
    ...
    scheduler.addTask({ type: TaskType.CUSTOM, priority: TaskPriority.HIGH, data: { name: 'HIGH' } });
    scheduler.addTask({ type: TaskType.CUSTOM, priority: TaskPriority.NORMAL, data: { name: 'NORMAL' } });
  ...
  it('应该支持任务依赖管理 (DAG)', async () => {
    ...
    const id2 = scheduler.addTask({ type: TaskType.CUSTOM, id: '2', data: { name: 'task2' }, dependencies: [id1] });
    scheduler.addTask({ type: TaskType.CUSTOM, id: '3', data: { name: 'task3' }, dependencies: [id2] });
